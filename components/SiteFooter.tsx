...
export function SiteFooter() {
  ...
  return (
    <footer className="bg-cream-deep border-t border-rule">
      <div className="max-w-[1320px] mx-auto px-7 pt-12 pb-7">
        <div className="grid grid-cols-[1.4fr_1fr_1fr_1fr] gap-12 pb-10 border-b border-rule">
          <div>
            <Link href="/" className="flex items-center gap-3 text-navy mb-4">
              <TechEchelonMark size={36} />
              <div className="font-display font-extrabold text-[30px] tracking-[-0.025em] text-navy leading-none">
                TechEchelon
              </div>
            </Link>
          ...
          {(Object.keys(SECTIONS) as Array<keyof typeof SECTIONS>).map((heading) => (
            <div key={heading}>
              <h4 className="font-mono text-[10.5px] font-bold tracking-[0.14em] uppercase text-coral mb-4">
                ━ {heading}
              </h4>
        ...
        <div className="flex justify-between font-mono text-[10.5px] tracking-[0.06em] uppercase font-semibold text-sand pt-6">
