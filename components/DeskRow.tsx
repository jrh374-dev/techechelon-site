    ...
    <section className="border-b border-rule">
      <div className="max-w-[1320px] mx-auto px-7 py-9">
        <div className="flex items-baseline gap-4 mb-6">
          <span className="font-mono text-[10px] tracking-[0.14em] uppercase font-bold text-sage">
            ━━ DESK
          </span>
          <h2 className="font-display text-[24px] font-extrabold tracking-[-0.022em] text-navy leading-none">
            {categoryLabel(desk)}
          </h2>
          <span className="flex-1 border-b border-rule mb-1.5" />
          <Link
            href={`/category/${desk}`}
            className="font-mono text-[10.5px] tracking-[0.08em] uppercase font-bold text-coral"
          >
            All {desk === "ai" ? "AI" : desk} →
          </Link>
        </div>
        <div className="grid grid-cols-[1.4fr_1fr_1fr] gap-10">
          <article className="group">
            <Link href={`/post/${lead.slug}`} className="block">
              ...
              <h3 className="font-display text-[26px] font-extrabold tracking-[-0.025em] leading-[1.08] text-ink mb-3 group-hover:text-navy">
                {lead.title}
              </h3>
              <p className="font-serif text-[14.5px] leading-snug text-ink-soft mb-3">
                {lead.excerpt}
              </p>
              <div className="flex items-baseline gap-2 font-mono text-[10px] tracking-[0.06em] uppercase text-sand">
          ...
          {rest.slice(0, 2).map((p) => (
            <article key={p.slug} className="group">
              <Link href={`/post/${p.slug}`} className="block">
                ...
                <h4 className="font-display text-[19px] font-extrabold tracking-[-0.02em] leading-[1.12] text-ink mb-2.5 group-hover:text-navy">
                  {p.title}
                </h4>
